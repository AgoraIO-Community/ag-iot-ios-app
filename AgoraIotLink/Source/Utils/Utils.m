...
+ (CVPixelBufferRef)i420ToNV12Image:(unsigned char *)srcY srcU:(unsigned char *)srcU srcV:(unsigned char *)srcV yStride:(int)yStride uStride:(int)uStride vStride:(int)vStride width:(int)width height:(int)height  {
...
+ (CVPixelBufferRef)i420ToPixelBuffer:(unsigned char *)srcY srcU:(unsigned char *)srcU srcV:(unsigned char *)srcV yStride:(int)yStride uStride:(int)uStride vStride:(int)vStride width:(int)width height:(int)height {
    ...
    CVReturn result = CVPixelBufferCreate(kCFAllocatorDefault,
                                          width,
                                          height,
                                          kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange,   //  NV12 //kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange
                                          (__bridge CFDictionaryRef)(pixelAttributes),
                                          &pixelBuffer);
    if (result != kCVReturnSuccess) {
