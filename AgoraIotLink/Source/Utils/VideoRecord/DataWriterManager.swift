...
public class DataWriterManager: NSObject {
    ...
    //MARK: ----- property
    var recordVideoFolder: String {//NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.cachesDirectory, FileManager.SearchPathDomainMask.userDomainMask, true).first
	...
	@objc func stopWrite() {
		...
		timer?.invalidate()
		timer = nil
        print("stopWrite: \(String(describing: assetWriter?.status))")
...
//	}
	...
	private func setUpWriter() {
        ...
        if documentPath == ""{
            ...
            videoUrl = URL.init(fileURLWithPath: documentPath)
        }
