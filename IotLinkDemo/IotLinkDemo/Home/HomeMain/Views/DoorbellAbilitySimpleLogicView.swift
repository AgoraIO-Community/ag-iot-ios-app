...
class DoorbellAbilitySimpleLogicView: UIView {
    ...
    var tipType : VideoAlertTipType{
        didSet{
            switch tipType{
            case .none:
                topControlView.tipsLabel.text = "deviceClosed".L + "..."
                topControlView.memberLabel.text = "number".L + ":0"
            case .loading:
                self.isOnCalling = false
                topControlView.tipsLabel.text = "in call".L + "..."
                topControlView.memberLabel.text = "number".L + ":0"
            case .loadFail:
                topControlView.tipsLabel.text = "call failed".L + "..."
            case .deviceOffLine:
                self.isOnCalling = false
                topControlView.tipsLabel.text = "deviceOffline".L
            case .deviceSleep:
                self.isOnCalling = false
                topControlView.tipsLabel.text = "deviceSleeping".L
            case .playing:
                self.isOnCalling = true
                topControlView.tipsLabel.text = "onCall".L
            }
        }
    }
